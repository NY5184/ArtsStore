...
const{title,description,price,imageUrl,createdAt,quantity}=req.body
if(!title||!description||!price||!imageUrl){
    return res.status(400).json({message:"all fileds is required"})
}
      
const newArt={title,description,price,imageUrl,createdAt,quantity}
...
const updateArt=async (req,res)=>{
    const{_id,title,description,price,imageUrl,createdAt,quantity}=req.body
    if(!_id){
